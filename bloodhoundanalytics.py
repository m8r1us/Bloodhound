...
class FrontPage(object):
    ...
    def create_qa_statistics(self, sheet):
        ...
        query = """MATCH (u:User {domain:$domain})
                    ...
                    RETURN 100.0 * COUNT(DISTINCT u) / userCount as percent
                    """
        ...
        query = """MATCH (c:Computer {domain:$domain})
                    ...
                    RETURN 100.0 * COUNT(c) / ComputerCount as percent          
                    """
        print (query)
        ...
        query = """MATCH(n:User{domain:$domain})
                    WHERE n.name =~ 'GUEST@.*'
                    RETURN n.enabled
                    """
        Guest_account = ""
        ...
        query = """MATCH p=(n)-[r]->(m{domain:$domain}) WHERE NOT n.domain = m.domain RETURN count(n.name)"""
        cross_domain_control = ""
        for result in session.run(query, domain=self.domain):
            cross_domain_control = result[0]
        ...
        self.write_single_cell(
            sheet, 8, 3, "Cross Domain objects found: {}".format(cross_domain_control))
...
class LowHangingFruit(object):
    ...
    def shortest_acl_path_domain_users(self, sheet):
        count_query = """MATCH (g1:Group {domain:$domain})
                        WHERE g1.objectid ENDS WITH "-513"
    ...
    def kerberoastable_path_len(self, sheet):
        list_query = """MATCH (u:User {domain:$domain,hasspn:true})
                        MATCH (g:Group {domain:$domain})
                        WHERE g.objectid ENDS WITH "-512" AND NOT u.name STARTS WITH "KRBTGT@"
...
class CriticalAssets(object):
    ...
    def do_critical_asset_analysis(self):
        func_list = [
            ...
            self.da_equiv_sessions, self.gpo_on_da_equiv, self.dcsync, self.highestprivgroups, self.shortestpathtoDA
        ]
    ...
    def admins_on_dc(self, sheet):
        list_query = """MATCH (g:Group {domain:$domain})
                    ...
                    OPTIONAL MATCH (n)-[:AdminTo|:Owns|:GenericAll|:HasSession|:CanRDP|:GetChangesAll]->(c)
                    OPTIONAL MATCH (m)-[:MemberOf*1..]->(:Group)-[:AdminTo]->(c)
    ...
    def highestprivgroups(self, sheet):
        list_query = """MATCH (g:Group{domain:$domain})-[:AdminTo]->(c:Computer)
                        RETURN g.name as GroupName,COUNT(c) as adminCount ORDER BY adminCount DESC"""
        session = self.driver.session()
        results = []
        ...
        session.close()
        self.write_column_data(
            sheet, "Highest Privileged Groups: {}", results)

    def shortestpathtoDA(self, sheet):
        list_query = """MATCH p=shortestPath((n)-[:MemberOf|HasSession|AdminTo|AllExtendedRights|AddMember|ForceChangePassword|GenericAll|GenericWrite|Owns|WriteDacl|WriteOwner|CanRDP|ExecuteDCOM|AllowedToDelegate|ReadLAPSPassword|Contains|GpLink|AddAllowedToAct|AllowedToAct|SQLAdmin|ReadGMSAPassword|HasSIDHistory|CanPSRemote*1..6]->
                        (m:Group {domain:$domain}))
                        WHERE m.objectid ENDS WITH "-512" AND NOT n=m RETURN n.name"""
        session = self.driver.session()
        results = []
        ...
        session.close()
        self.write_column_data(
            sheet, "Shortestpath to DA path length of 6: {}", results)
...
class CrossDomain(object):
    ...
    def do_cross_domain_analysis(self):
        func_list = [
            ...
            self.foreign_user_controllers, self.foreign_session, self.weak_trusts, self.foreign_objects
        ]
    ...
    def foreign_session(self, sheet):
        list_query = """MATCH (s:Computer)-[r:HasSession*1]->(t:User{domain:$domain})
        ...
        session.close()
        self.write_column_data(
            sheet, "Users with Foreign Session: {}", results)

    def weak_trusts(self, sheet):
        list_query = """MATCH p=(n:Domain{domain:$domain})-[r]-(m:Domain) where r.sidfiltering=false RETURN DISTINCT(m.name)"""
        ...
        session.close()
        self.write_column_data(
            sheet, "Weak trusts - SID Filtering disabled: {}", results)

    def foreign_objects(self, sheet):
        list_query = """MATCH (n)-[r]->(m{domain:$domain}) WHERE NOT n.domain = m.domain RETURN LABELS(n)[1],n.name,TYPE(r),LABELS(m)[1],m.name
                        """
        session = self.driver.session()
        results = []

        for result in session.run(list_query, domain=self.domain):
            results.append("{} - {} - {} - {} - {}".format(result[0], result[1], result[2], result[3], result[4]))

        session.close()
        self.write_column_data(
            sheet, "Foreign objects: {}", results)
...
class Organization(object):
    ...
    def da_members(self, sheet):
        list_query = """MATCH (n:Group {domain:$domain})
                            WHERE n.objectid =~ "(?i)S-1-5.*-512" WITH n
